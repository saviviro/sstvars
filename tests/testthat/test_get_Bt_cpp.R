...
test_that("get_Bt_Cpp calculates correct weighted sums", {
  ...
  alpha_mt <- matrix(c(1, 2)^2, nrow=1) # Single row of weights
  ...
  alpha_mt <- matrix(c(1, 0.5, 2, 3)^2, nrow=2) # Two rows of weights
  ...
  alpha_mt <- matrix(c(0.5, 0.3, 0.2)^2, nrow=1) # Single row of weights

  expected <- array(c(3.8, 4.8, 5.8, 6.8), dim=c(2, 2, 1))
  result <- get_Bt_Cpp(all_Omegas, alpha_mt)
  expect_true(all.equal(result, expected))

  alpha_mt <-  rbind(c(0.5, 0.3, 0.2), c(0, 0, 1), c(0.1, 0.2, 0.7))^2 # Three rows of weights
  expected <- array(c(3.8, 4.8, 5.8, 6.8, 9, 10, 11, 12, 7.4, 8.4, 9.4, 10.4), dim=c(2, 2, 3))
  result <- get_Bt_Cpp(all_Omegas, alpha_mt)
  expect_true(all.equal(result, expected))

  alpha_mt <-  rbind(c(0.5, 0.3, 0.2), c(0, 0, 1), c(0.1, 0.2, 0.7), c(0.8, 0, 0.2))^2 # Four rows of weights
...
test_that("get_Bt_Cpp handles single matrix and weight correctly", {
  ...
  alpha_mt <- matrix(1, nrow=1)^2 # Single weight
...
test_that("get_Bt_Cpp output has correct dimensions", {
  ...
  alpha_mt <- matrix(rnorm(6), nrow=2)^2 # Two sets of weights
