...
test_that("iterate_more works correctly", {
  ...
  fit12cm <- iterate_more(fit12cm, maxit=30, calc_std_errors=FALSE)
...
test_that("fitSSTVAR works correctly", {
  fit12cm_rec <- fitSSTVAR(fit12cm, new_identification="recursive", calc_std_errors=FALSE)
  expect_equal(fit12cm_rec$params, c(0.833884628, 0.475115791, 0.309214073, 0.019662000, -0.120540749, 0.886574549, 0.947106362,
                                 -0.034873197, 0.151487900, 0.319143635, 0.015522353, 0.009552297, 0.417772934), tolerance=1e-4)

  fit12cm_hsked <- fitSSTVAR(fit12cm, new_identification="heteroskedasticity", calc_std_errors=FALSE)
  ...
  fit12cm_hsked2 <- fitSSTVAR(fit12cm_hsked, new_identification="heteroskedasticity", maxit=100, maxit_robust=100, print_res=FALSE,
                              new_B_constraints=matrix(c(1, 0, NA, 1), nrow=2), robust_method="Nelder-Mead", calc_std_errors=FALSE)
