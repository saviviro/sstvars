...
diagnostic_plot <- function(stvar, type=c("all", "series", "ac", "ch", "dist"), resid_type=c("standardized", "raw"),
  ...
  if(type == "dist" || type == "all") {
    if(stvar$model$cond_dist != "Gaussian") stop("Only Gaussian cond_dist is currently implemented to diagnostic_plot")
    # KUN TULEE STUDENT NIIN MUISTA LAITTAA TS JA DIST PLOTTEIHIN TIETO JAKAUMASTA JOHONKIN
    par(mfrow=c(2, d), mar=c(2.5, 2.8, 2.1, 1.0))
    for(i1 in 1:d) {
      hs <- hist(res[,i1], breaks="Scott", probability=TRUE, col="skyblue", plot=TRUE,
                 main=colnames(res)[i1], ylim=c(0, 0.5))
      x <- seq(from=min(hs$breaks), to=max(hs$breaks), length.out=1000)
      lines(x=x, y=dnorm(x), lty=2, col="darkred")
    }
    for(i1 in 1:d) {
      qqnorm(res[,i1], main="")
      qqline(res[,i1], col="darkred")
    }
  }
}
