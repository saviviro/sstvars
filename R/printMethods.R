...
#' @title Print method for the class histdecomp
#'
#' @description \code{print.hypotest} is the print method for the class histdecomp
#'  objects.
#' @param digits how many significant digits to print?
...
print.histdecomp <- function(x, ..., digits=3, which_vars, which_indices) {
  ...
  if(missing(which_vars)) {
    which_vars <- 1:histdec$stvar$model$d
  } else {
    stopifnot(is.numeric(which_vars) && length(which_vars) <= histdec$stvar$model$d && all(which_vars %in% 1:histdec$stvar$model$d))
